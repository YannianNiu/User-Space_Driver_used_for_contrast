...
struct mempool* memory_allocate_mempool(struct device* dev, uint32_t num_entries, uint32_t entry_size) {
	...
	//debug("allocate mempool，number of memory block is :%d,size of memory block is :%d",num_entries,entry_size);
...
static struct mempool* init_mempool(struct device* dev, uint32_t num_entries, uint32_t entry_size) {
	...
	struct mempool* mempool = memory_allocate_mempool(dev, num_entries, 0);
...
uint32_t transmit(struct device* dev, struct pkt_buf* bufs[], uint32_t num_bufs) {
	...
	//uint64_t this_time = monotonic_time();
	//uint64_t time_diff = this_time - last_time;
	//debug("%ld",time_diff);
	//debug("read_u32(dev, GPTC) = %d",read_u32(dev, GPTC));
	return sent;
}
...
static void refull_mempool(struct mempool* mempool, uint32_t num_entries) {
	const int NUM_BUFS = num_entries;
	struct pkt_buf* bufs[NUM_BUFS];
	//struct timespec tim;
	//tim.tv_sec = 0;
...
static void refull_mempool1(struct mempool* mempool, uint32_t num_entries) {
	const int NUM_BUFS = num_entries;
	struct pkt_buf* bufs[NUM_BUFS];
	//struct timespec tim;
	//tim.tv_sec = 0;
	...
	for (int buf_id = 0; buf_id < NUM_BUFS; buf_id++) {
		...
		buf->size = 256;
		uint64_t last_time = monotonic_time();
		//memcpy(buf->data, pkt_data1, sizeof(pkt_data1));
		for(int i=0;i<3;i++)
	      	{
			strncat(buf->data, pkt_data1, 256);
			buf->size += 256;
			uint64_t this_time = monotonic_time();
			if(monotonic_time() - last_time > time_batch)
			    break;
...
int main(int argc, char* argv[]) {
    ...
    uint64_t last_time = monotonic_time();
    struct mempool* mempool = init_mempool(dev, 4096, 0);
    ...
    while (true) {
        if(sent_num >= 4096){
            uint64_t this_time = monotonic_time();
            if(this_time - last_time > 2000 * 1000 * 1000){
                flag++;
                last_time = this_time;
            }
            if(flag % 2 == 0)
                refull_mempool(mempool, 3000);
            else
                refull_mempool1(mempool,3000);
            sent_num = 0;
        }
        pkt_buf_alloc_batch(mempool, bufs, BATCHSIZE); // 64        
        tx_batch_busy_wait(dev, bufs, BATCHSIZE);
